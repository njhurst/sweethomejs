...
[ ] 1  P0 Foundations: tooling, corpus, spikes, geometry shim
    [ ] 1.1  Scaffold monorepo: Vite, tsup, Vitest, Playwright, ESLint/Prettier, strict TS, CI jobs
    [ ] 1.2  Set up package layout (core/render2d/render3d/ui/photo/codecs/apps per 03-porting-strategy.md)
...
[ ] 3  P2 File format: .sh3d read/write with round-trip parity (05-file-format.md)
    ...
    [ ] 3.5  ContentDigestManager + ContentDigests manifest read/write + damaged-content repair flow
    ...
    [ ] 3.8  Round-trip identity tests + Java↔JS cross-read field-dump parity + XML byte parity (incl. entry-name order)
...
[ ] 5  P4 2D plan view: PlanComponent → Canvas2D/SVG (06-2d-plan-view.md)
    [ ] 5.1  PlanPainter interface + Canvas2D implementation + SVG implementation
    [ ] 5.2  Viewport/transform module: scale, pan, zoom-at-point, HiDPI (devicePixelRatio), plan bounds + margin
    [ ] 5.3  Paint-order pipeline: background, grid, rulers, levels, rooms, walls, dims/labels/polylines, furniture, compass, scale bar
